	...
	void ParseCInclude(ama::Node* nd_root) {
		...
		for ( ama::Node * & nd_raw: nd_root->FindAllWithin(0, ama::N_RAW, nullptr) ) {
			...
			if ( !nd_raw->c->s || nd_raw->c->s->s || nd_raw->c->s->node_class != ama::N_STRING ) {
				...
				ama::Node* nd_real_include = ama::nString(JC::array_cast<JC::unique_string>(include_expr))->setCommentsBefore(
					JC::array_cast<JC::unique_string>(comments_before)
				)->setCommentsAfter(
					JC::array_cast<JC::unique_string>(comments_after)
				);
	...
	void ParseJSRequire(ama::Node* nd_root) {
		...
		for ( ama::Node * & nd_dep: nd_root->FindAllWithin(0, ama::N_DEPENDENCY, nullptr) ) {
			if ( (nd_dep->flags & ama::DEP_TYPE_MASK) == ama::DEP_JS_REQUIRE && nd_dep->data == nullptr ) {
				...
				for ( JC::unique_string const& dir: js_require_paths ) {
					fn_commonjs = ama::FindCommonJSModuleByPath(JC::array_cast<JC::unique_string>(path::normalize(JC::string_concat(dir, path::sep, fn_required))));
					if ( fn_commonjs != nullptr ) { break; }
				}
